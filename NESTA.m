...
%
% Solves the filtering norm L1 and/or L2 minimization problem under a quadratic constraint using the
% Nesterov algorithm, with continuation:
%
%     min_x ||H x ||_1 s.t. ||y - Ax||_2 <= delta
% 
...
%                   'filters' for filter norm, if all filters 
%                   unidimensional, it applies 1D filter solution
%                   If at least one of the filters are bidimensional
%                   or are in oposite directions (for instance, 
%                   [1 -1] and [1 -1]'), it applies the 2D filter norm.
%               opts.h - cell array of filter coefficients, used for 
%                   'filters' option above.
%               opts.alphaIso - weigth of Isotropic filter norm (default is 1)
%               opts.alphaAni - weigth of Anisotropic filter norm (default is 0)
%                   If one of alphas is zero, is applied only the other
%                   filter norm. Both cannot be 0.
...
alphaIso = setOpts('alphaIso',1);
alphaAni = setOpts('alphaAni',0);
...
switch lower(TypeMin)
    ...
    case 'filters'
        maxHx=0;
        maxHy=0;
        for idx=1:length(opts.h)
            [size1, size2]=size(opts.h{idx});
            if size1>maxHx; maxHx=size1; end
            if size2>maxHy; maxHy=size2; end
        end
        if maxHx==1 || maxHy==1
            opts.filtersDim=1;
            [mu0,opts.H,opts.D] = ValMUHDFilters1D(Ux_ref,opts.h);
        else
            opts.filtersDim=2;
            [mu0,opts.H,opts.D] = ValMUHDFilters2D(Ux_ref,opts.h);
        end   
        %aniso weigth is based on mu0 of l1 case, after applying D, while
        %iso is from ValMUHDFilters2D
        mu0 = alphaAni*0.9*max(abs(opts.D*Ux_ref))+alphaIso*mu0;
end
...
for nl=1:MaxIntIter
    ...
    if Verbose
        ...
        if strcmp(opts.TypeMin,'filters')
            printf('\tAlpha isotropic=%f; alpha anisotropic=%f\n',opts.alphaIso,opts.alphaAni);
            printf('\tFilter coefficients:\n');            
            for iii=1:length(opts.h)
                printf('\t___Filter h%d:\n',iii);            
                disp(opts.h{iii});
            end
...
%---- internal routine for setting mu0, H and D in the general filters minimization case
function [th,H,D]=ValMUHDFilters1D(x,h)
    p=2;
    N = length(x);
...
%---- internal routine for setting mu0 in the general filters minimization case
function [th,H,D]=ValMUHDFilters2D(x,h)
    p=2;
    N = length(x);n = floor(sqrt(N));
