...
% Modified: Sep 2018, Jonathan Lima, UnB/UTEP
% NESTA Version 2.0
%   See also NESTA
...
%---- Filters Minimization
if strcmpi(TypeMin,'filters')
    if opts.filtersDim==2
        Lmu = 8*Lmu;
        if ~(isfield(opts,'H') && isfield(opts,'D'))
            [H,D]=filter2matrix(opts.h,n,n);
        else
            H=opts.H;
            D=opts.D;
        end
    elseif opts.filtersDim==1
        if ~(isfield(opts,'H') && isfield(opts,'D'))
            [H,D]=filter1D2matrix(opts.h,N);
        else
            H=opts.H;
            D=opts.D;
        end
...
for k = 0:maxiter-1
   ...
   if strcmpi(TypeMin,'filters')
        if alphaIso==0 && alphaAni==0
            error('alphaIso and alphaAni must not be both 0');
        end
        if alphaAni==0
            [df,fx] = Perform_Filters_Constraint(xk,mu,H,D,U,Ut);
            fx=alphaIso*fx;
        end
        
        if alphaIso==0
            [df,fx] = Perform_L1_Constraint(xk,mu,DU,DUt);
            fx=alphaAni*fx;
        end
        %TODO check df
        if alphaAni~=0 && alphaIso~=0
            [dfIso,fx1] = Perform_Filters_Constraint(xk,mu,H,D,U,Ut);
            [dfAni,fx2] = Perform_L1_Constraint(xk,mu,DU,DUt);
            df=alphaIso*dfIso+alphaAni*dfAni;
            fx=alphaIso*fx1+alphaAni*fx2;
        end
    end
...
%%%%%%%%%%%% PERFORM THE FILTERS CONSTRAINT %%%%%%%%%%%%%%%%%%
%%%% same for 1D and 2D filters ???
function [df,fx] = Perform_Filters_Constraint(xk,mu,H,D,U,Ut)
p=2;
if isa(U,'function_handle')
    x = U(xk);
else
